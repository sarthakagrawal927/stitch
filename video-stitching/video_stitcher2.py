    ...
    def stitch(self, images, ratio=0.75, reproj_thresh=4.0):
        ...
        if self.saved_homo_matrix is None:
            ...
            visual = self.draw_matches(
                image_a, image_b, keypoints_a, keypoints_b, matched_keypoints[0], matched_keypoints[2])
            cv2.imshow("KeyPoints", imutils.resize(visual, width=1000))
...
stitcher = VideoStitcher(left_video_in_path='sample_1.mp4',
                         ...
                         video_out_path='sample_output_2.mp4', display=False)
stitcher.run()
